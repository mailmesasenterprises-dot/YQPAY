...
const TheaterPaymentGatewaySettings = () => {
  ...
  const fetchTheaters = async () => {
    try {
      ...
      const theatersList = Array.isArray(response.data) ? response.data : 
                          (response.data.theaters || response.data.data || []);
  ...
  const fetchTheaterConfig = async (theaterIdToFetch) => {
    try {
      setLoading(true);
      const token = config.helpers.getAuthToken();
      const response = await axios.get(`/api/theaters/${theaterIdToFetch}`, {
        headers: token ? { 'Authorization': `Bearer ${token}` } : {}
      });
