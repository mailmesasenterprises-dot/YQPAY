  ...
  const fetchTheaters = useCallback(async () => {
    try {
      ...
      const params = new URLSearchParams({
        ...
        // Show all theaters (active and inactive) for management purposes
      });
      ...
      const cacheKey = `theaters_list_${currentPage}_${itemsPerPage}_${debouncedSearchTerm}_${filterStatus}`;
      
      const result = await optimizedFetch(cacheBustedUrl, {
        signal: abortControllerRef.current.signal,
        headers: {
          'Accept': 'application/json',
          ...config.helpers.getAuthToken() ? { 'Authorization': `Bearer ${config.helpers.getAuthToken()}` } : {}
        }
      }, cacheKey, 60000); // 1 minute cache for theater list
