...
const RoleNameManagement = () => {
  ...
  const { theaterId: pathTheaterId } = useParams(); // Get from path (/role-names/:theaterId)
  ...
  const toggleRoleStatus = async (roleId, currentStatus) => {
    try {
      ...
      const response = await fetch(`${config.api.baseUrl}/role-names/${roleId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
  ...
  const loadRoleData = useCallback(async () => {
    try {
      ...
      const baseUrl = `${config.api.baseUrl}/role-names?${params.toString()}`;
  ...
  const handleSubmitRole = async (isEdit = false) => {
    try {
      ...
      const url = isEdit 
        ? `${config.api.baseUrl}/role-names/${selectedRole._id}` 
        : `${config.api.baseUrl}/role-names`;
      ...
      if (response.ok) {
        ...
        showSuccess && showSuccess(
          isEdit 
            ? `Email Notification "${formData.emailNotification}" updated successfully` 
            : `Email Notification "${formData.emailNotification}" created successfully`
        );
      } else {
        const errorData = await response.json().catch(() => ({}));

        throw new Error(errorData.error || `Failed to save email notification: ${response.status}`);
      }
    } catch (error) {

      showError && showError(`Failed to save email notification: ${error.message}`);
    }
  };
  ...
  const handleDeleteRole = async () => {
    try {
      ...
      const response = await fetch(`${config.api.baseUrl}/role-names/${selectedRole._id}?permanent=true`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok) {
        ...
        loadRoleData(); // Refresh the list
        showSuccess && showSuccess(`Email Notification "${selectedRole.emailNotification}" permanently deleted`);
      } else {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || `Failed to delete email notification: ${response.status}`);
      }
    } catch (error) {

      showError && showError(`Failed to delete email notification: ${error.message}`);
    }
  };
