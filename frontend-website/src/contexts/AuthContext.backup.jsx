...
export const AuthProvider = ({ children }) => {
  ...
  const [rolePermissions, setRolePermissions] = useState([]); // Add role-based permissions (page-level)
  const [permissions, setPermissions] = useState([]); // Add permissions array (action-level)
  ...
  const login = (userData, token, type = 'super_admin', userTheaterId = null, userRolePermissions = [], userPermissions = []) => {
    ...
    // Store role permissions for theater users (page-level)
    ...
    // Store permissions for action-level access control
  ...
  const hasPermission = (permission) => {
    ...
    if (userType === 'super_admin') return true; // Super admin has all permissions
