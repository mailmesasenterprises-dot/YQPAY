  ...
  useEffect(() => {
    const loadConfigurations = async () => {
      try {
        ...
        const firebaseData = await optimizedFetch(
        ...
        // Load MongoDB config
        const mongoData = await optimizedFetch(
        ...
        const gcsData = await optimizedFetch(
        ...
        // Load SMS config
        const smsData = await optimizedFetch(
        ...
        // Load Mail config
        const mailData = await optimizedFetch(
        ...
        if (mailData) {
          const config = mailData.data || {};
          // Always merge config, even if some values are empty
          if (config) {
            setMailConfig(prevConfig => {
              const merged = { ...prevConfig, ...config };
              return merged;
            });
          }
        ...
        const generalData = await optimizedFetch(
                ...
                <div className="form-group full-width">
                  ...
                  <div style={{ display: 'flex', gap: '24px', marginTop: '8px' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                      ...
                      />
                      <span style={{ fontSize: '14px', fontWeight: '500' }}>SSL</span>
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                      <input
                        type="radio"
                        ...
                        style={{ width: '18px', height: '18px', cursor: 'pointer' }}
                      />
                      <span style={{ fontSize: '14px', fontWeight: '500' }}>TLS</span>
                    </label>
                  </div>
                </div>
