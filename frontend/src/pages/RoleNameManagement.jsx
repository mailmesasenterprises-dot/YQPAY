...
const RoleNameManagement = () => {
  ...
  const handleSubmitRole = async (isEdit = false) => {
    try {
      ...
      if (response.ok) {
        ...
        const errorMessage = errorData.error || errorData.message || `Failed to save email notification: ${response.status}`;
        throw new Error(errorMessage);
      }
    } catch (error) {
      console.error('Submit role error:', error);
      showError(`Failed to save email notification: ${error.message}`);
    }
  };
  ...
  const handleDeleteRole = async () => {
    try {
      ...
      const response = await fetch(`${config.api.baseUrl}/email-notification/${selectedRole._id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok) {
        ...
        const errorData = await response.json().catch(() => ({}));
        const errorMessage = errorData.error || errorData.message || `Failed to delete email notification: ${response.status}`;
        throw new Error(errorMessage);
      }
    } catch (error) {
      console.error('Delete role error:', error);
      showError(`Failed to delete email notification: ${error.message}`);
    }
  };
