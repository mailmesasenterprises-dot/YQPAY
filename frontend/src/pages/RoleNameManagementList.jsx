...
const RoleNameManagementList = () => {
  const navigate = useNavigate();
  
  // PERFORMANCE MONITORING: Track page performance metrics
  usePerformanceMonitoring('RoleNameManagementList');
  
  // Cache key constant
  const getCacheKey = (page, limit, search) => 
    `theaters_role_name_management_page_${page}_limit_${limit}_search_${search || 'none'}_active`;
  
  // 🚀 OPTIMIZED: Check cache synchronously on mount for instant loading
  // Check cache for initial page load (page 1, no search)
  ...
  // State management (exactly like RoleManagementList)
  ...
  const fetchTheaters = useCallback(async () => {
    try {
      ...
      if (!result) {
        throw new Error('Failed to fetch theaters for role name management');
      }
      ...
      if (result.success) {
        ...
        throw new Error(result.message || 'Failed to fetch theaters for role name management');
      }
      ...
      setError('Failed to load theaters for email notification management');
    } finally {
      setLoading(false);
    }
  ...
  const handleRoleNameManagementClick = (theater) => {
    navigate(`/email-notification/${theater._id}`);
  };
  ...
  if (error) {
    return (
      <AdminLayout pageTitle="Email Notification Management" currentPage="email-notification">
            ...
            <div className="theater-content">
              ...
              {sortedTheaters.length === 0 && !loading ? (
                <div className="empty-state">
                  ...
                  <p>There are no theaters available for email notification management at the moment.</p>
                </div>
              ) : (
                            ...
                            <tr key={theater._id} className="theater-row">
                              ...
                              <td className="actions-cell">
                                <ActionButtons>
                                  <ActionButton 
                                    type="view"
                                    onClick={() => handleRoleNameManagementClick(theater)}
                                    title="Manage Email Notifications for this Theater"
                                  />
                                </ActionButtons>
                              </td>
                            </tr>
...
export default RoleNameManagementList;
