...
const TheaterUsersArray = () => {
  ...
  /**
   * Load users for selected theater
   */
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/theater-users?${queryParams}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  }, [selectedTheater, pagination.currentPage, pagination.limit, searchTerm]);
