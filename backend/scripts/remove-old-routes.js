...
console.log('🗑️  Old Route Files Cleanup Script\n');
console.log('='.repeat(50));
console.log('⚠️  WARNING: This will move old route files to backup!');
console.log('⚠️  Make sure all MVC routes are tested and working!\n');
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (mvcFileExists) {
      try {
        ...
        console.log(`✅ Moved: ${routeFile} → _old_backup/${routeFile}`);
        movedCount++;
      } catch (error) {
        console.error(`❌ Failed to move ${routeFile}:`, error.message);
      }
    } else {
      console.log(`⚠️  Skipped: ${routeFile} (MVC file not found)`);
      skippedCount++;
    }
  } else {
    console.log(`ℹ️  Already removed: ${routeFile}`);
  }
});
...
console.log('\n💡 To restore, move files back from _old_backup/');
console.log('='.repeat(50));
