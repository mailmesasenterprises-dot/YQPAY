...
async function checkIndexes() {
  try {
    console.log('🔍 Checking QRCodeNames indexes...\n');
    ...
    if (nullTheaterDocs.length > 0) {
      ...
      nullTheaterDocs.forEach(doc => {
        console.log(`  - _id: ${doc._id}, theaterId: ${doc.theaterId}, qrNames: ${doc.qrNameList?.length || 0}`);
      });
    }
    ...
    if (theaterIdDocs.length > 0) {
      console.log('\n📋 Documents with theaterId:');
      theaterIdDocs.forEach(doc => {
        console.log(`  - _id: ${doc._id}, theaterId: ${doc.theaterId}, theater: ${doc.theater}`);
      });
    }
